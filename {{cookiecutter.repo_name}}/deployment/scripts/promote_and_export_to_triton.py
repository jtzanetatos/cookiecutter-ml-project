...
def _resolve_signature(model_uri: str) -> Any:
    ...
    if mlmodel.signature is None:
        raise RuntimeError(f"Model at {model_uri} has no signature; cannot infer Triton config.")
    return mlmodel.signature


def _infer_io_from_signature(signature: Any) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
    ...
    for field in signature.inputs:
        ...
        inputs.append({"name": name, "dtype": dtype, "shape": list(shape) if shape else []})

    for field in signature.outputs:
        name = getattr(field, "name", "output")
        dtype = str(getattr(field, "type", getattr(field, "dtype", "float32"))).lower()
        shape = getattr(field, "shape", None)
        outputs.append({"name": name, "dtype": dtype, "shape": list(shape) if shape else []})
...
def _write_config_pbtxt(
    ...
    in_lines: List[str] = []
    for i in inputs:
        dims = _triton_dims(i.get("shape", []), max_batch_size) if i.get("shape") else []
    ...
    out_lines: List[str] = []
    for o in outputs:
        dims = _triton_dims(o.get("shape", []), max_batch_size) if o.get("shape") else []
        dims_str = ", ".join(str(d) for d in dims) if dims else "-1"
        out_lines.append(
...
def _export_sklearn_to_onnx(
    ...
    if not inputs:
        raise RuntimeError("No inputs in model signature; cannot export sklearn model to ONNX.")

    # TensorSpec: infer feature count from last dim if available, else fallback
    if inputs[0].get("shape"):
        shape = inputs[0]["shape"]
        feature_dim = int(shape[-1]) if shape and shape[-1] not in (None, -1) else len(inputs)
    else:
...
def _export_torch_to_onnx(
    ...
    if not inputs or not inputs[0].get("shape"):
        raise RuntimeError("Torch ONNX export requires TensorSpec inputs with shapes in signature.")
...
def _write_history(output_dir: Path, triton_model_name: str, entry: Dict[str, Any]) -> None:
...
def _promote_alias(
    ...
    if archive_previous and prev_version and prev_version != version:
        client.set_registered_model_alias(name=model_name, alias=archived_alias, version=prev_version)
...
def promote_and_export(
    ...
    version: str = typer.Option(..., help="Target MLflow model version to export and promote."),
    tracking_uri: Optional[str] = typer.Option(None, help="MLflow tracking URI (prefer env var in CI)."),
    alias: str = typer.Option("prod", help="Lifecycle alias to set (e.g., prod)."),
    archived_alias: str = typer.Option("archived", help="Alias to use for previous prod (if archiving)."),
    archive_previous: bool = typer.Option(True, help="If true, archive previous aliased version."),
    output_dir: Path = typer.Option(Path("deployment/triton/model_repository"), help="Triton model_repository root."),
    triton_model_name: Optional[str] = typer.Option(None, help="Override Triton model directory name."),
    batch: Optional[int] = typer.Option(None, help="None/1 => no batching; >1 => max_batch_size=batch."),
    clean_version_dir: bool = typer.Option(True, help="If true, delete existing version dir before exporting."),
) -> None:
    """
    ...
    if archive_previous and prev_version and prev_version != version:
        typer.echo(f"Archived previous {alias}: v{prev_version} via alias '{archived_alias}'")
