...
# from {{cookiecutter.project_slug}}.inference.predictor import Predictor
# from {{cookiecutter.project_slug}}.schemas.requests import PredictRequest
...
with ui.column().classes("w-full items-start"):
    ...
    ui.label("Template skeleton: replace with project-specific content.").classes("text-sm text-gray-500")

    model_name = ui.input("MLflow model name", value=os.getenv("MLFLOW_MODEL_NAME", "CHANGE_ME"))
    model_alias = ui.input("Alias", value=os.getenv("MLFLOW_MODEL_ALIAS", "prod"))

    ui.separator()

    ui.label("Inputs")
    inp = ui.textarea(placeholder="Enter JSON or CSV-like inputs (project-specific)").classes("w-full")
